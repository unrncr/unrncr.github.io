...
function ValidateEmail()
{
	...
	user = document.getElementById("RecoveryUsername").value;
	if(!xmlHttp)
	{
	...
	if(!user)
	{
		document.getElementById("RecoveryUsernameFault").innerHTML="This field is mandatory.";
		fault = 1;
	}
	else
		document.getElementById("RecoveryUsernameFault").innerHTML="";
	if(!email)
	{
	...
	{//validate email
		var regsite = "php/validateemailanduser.php?email="+email+"&user="+user;
...
function RecoverPassword()
{
	...
	{//send email for confirmation
		var regsite = "php/passwordreminder.php?user="+user+"&email="+email+"&answer="+answer;
		xmlHttp.open("GET",regsite,true);
		xmlHttp.setRequestHeader("If-Modified-Since", "Sat, 1 Jan 2000 00:00:00 GMT");
		xmlHttp.setRequestHeader("Cache-Control", "no-cache"); 
		if(!xmlHttp)
		...
		xmlHttp.onreadystatechange=function()
		{
			if(xmlHttp.readyState==4)
			{
				...
				{
					document.getElementById("RecoveryAnswerFault").innerHTML="Password sent to your email address!";
